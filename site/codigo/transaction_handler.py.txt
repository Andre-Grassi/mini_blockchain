"""Transaction handler for blockchain operations.

Validates and executes deposit/withdrawal transactions, creates appropriate
blocks (standard Block or AccCreationBlock for first deposits), and manages
client balances. Does not validate hashes - that responsibility belongs to
the Hash class. All methods are static as the class serves as a utility.
